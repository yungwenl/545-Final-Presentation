...
#Successful Rate by Service Type
```{r}
...
#Successful Rate by department(Owner)
```{r}
unique(data$Owner)
...
Owner%>%
  ...
  theme_light()
>>>>>>> 91aad8b01501fefacd7d81135a2c9a7411407ba4
```


#Successful Rate by Request Source
```{r}
RequestSourceTotal=
  data %>%
...
RequestSource = data %>%
  ...
  summarise(deliver = n())%>%
  filter(deliver>17)%>%
  mutate(total=RequestSourceTotal$total)%>%S
